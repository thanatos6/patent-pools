...
public class StatusCodeController {
    ...
    //管理员点击驳回按钮，改变专利当前状态，并且插入驳回列表一条信息，前端测试通过
    @Action(name = "reject")
    ...
    //根据认领人ID查看自己认领的待审批的专利 ，状态码为1,5,6，测试成功
    ...
    //根据当前用户ID查看自己认领的待维护的专利列表 ，状态码为7提交成功，测试成功
    ...
    //根据当前用户点击一审被驳回的专利列表，获取专利列表Id,改状态11变0,测试成功
    @RequestMapping(value = "/know", method = RequestMethod.POST)
    public String know(@RequestBody PatentInfo patentInfo) {
        if (patentInfo.getCurrentStatus() == null || patentInfo.getId() == null) {
            return ZhuanliUtil.getJSONString("传参为空");
        }
        boolean result = statusCodeService.updateStatusFinish(patentInfo);
        return ZhuanliUtil.getJSONString(200, result);
    }


    //撰写人点击查看驳回原因，显示专利被驳回理由，测试成功
    @RequestMapping(value = "/viewreason", method = RequestMethod.POST)
    public String know(@RequestBody RejectContent rejectContent) {
        if (rejectContent.getPatentId() == null) {
            return ZhuanliUtil.getJSONString("传参为空");
        }
